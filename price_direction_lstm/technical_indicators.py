...
def williams_ad(df):
    ...
    for index, row in df.iterrows():
        if index > 0:
            prev_value = df.at[index-1, 'williams_ad']
            ...
            if row['Close'] > prev_close:
                ...
                ad = 0.
            df.at[index, 'williams_ad'] = ad+prev_value
    return df


#use this one to replace smi
# https://github.com/kylejusticemagnuson/pyti/blob/master/pyti/stochrsi.py
def stochastic_relative_strength_index(df, n):
    """
    StochRSI.
    Formula:
    SRSI = ((RSIt - RSI LOW) / (RSI HIGH - LOW RSI)) * 100
    """
    rsi = _relative_strength_index(df, n)[n:]
...
def create_lags(df, lags = 7, columns = ['return']):
    for column in columns:
        for lag in range(1, lags + 1):
            col = '{}_lag_{}'.format(column, lag)
            lag = pd.Series(df[column].shift(lag), name=col)
            df= df.join(lag)
    return df
# def create_lags(df, lags = 7):
#     for lag in range(1, lags + 1):
#         col = 'lag_{}'.format(lag)
#         lag = pd.Series(df['return'].shift(lag), name='lag_' + str(lag))
#         df= df.join(lag)
#     return df
...
# def exponential_moving_average(df, n, alpha=0.9):
#     """
...
def _relative_strength_index(df, n):
...
def relative_strength_index(df, n):
    RSI = _relative_strength_index(df, n)
    df = df.join(RSI)
    return df
